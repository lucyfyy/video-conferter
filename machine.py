...
def copy_mp4(src_bucket_name, dest_bucket_name, filename):
    client = google.cloud.logging.Client()
    client.setup_logging()
    logging.info(f"Copying {filename} to {dest_bucket_name}")
    cloudstorage.download(src_bucket_name, filename)
    cloudstorage.upload(dest_bucket_name, filename, filename)
    logging.info(f"File {filename} successfully copied to {dest_bucket_name}")

def convert_webm(dest_bucket_name, name):
    client = google.cloud.logging.Client()
    client.setup_logging()
    ...
    ffmpeg.run(webm)
    cloudstorage.upload(dest_bucket_name, f"{name}.webm", f"{name}.webm")
...
def convert_ogg(dest_bucket_name, name):
    client = google.cloud.logging.Client()
    client.setup_logging()
    stream = ffmpeg.input("/tmp/{file}.mp4".format(file=name))
    ...
    ffmpeg.run(ogg)
    cloudstorage.upload(dest_bucket_name, f"{name}.ogg", f"{name}.ogg")
...
def create_thumbnail(dest_bucket_name, name):
    client = google.cloud.logging.Client()
    client.setup_logging()
    ...
    cloudstorage.upload(dest_bucket_name, f"{name}-thumb.jpg", f"{name}-thumb.jpg")
